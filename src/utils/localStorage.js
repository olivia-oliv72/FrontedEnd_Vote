// client/src/utils/localStorage.js (REVISI)

const CATEGORIES_CACHE_KEY = "award_categories_cache";
const HISTORY_CACHE_KEY_PREFIX = "user_history_cache_"; // Tambahkan prefix jika history per user

// Untuk kategori (sebagai cache)
export function getCachedCategories() {
  const stored = localStorage.getItem(CATEGORIES_CACHE_KEY);
  return stored ? JSON.parse(stored) : null; // Kembalikan null jika tidak ada cache
}
...
// Untuk history (sebagai cache, mungkin perlu email sebagai bagian dari key)
...
// localStorage tetap sangat berguna untuk hal lain, seperti:
